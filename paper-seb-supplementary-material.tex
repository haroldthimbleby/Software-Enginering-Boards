...
% pageref gives us a hyperlink which looks unsightly
\makeatletter
...
\changemysection{The Speigelhalter trustworthiness questions}
...
\begin{change}\sequestion{What evidence is there of rigorous (e.g., code review and tool-based) independent methods being used to manage coding bias?}\end{change}
...
\hbox{ }\hfill\begin{change}\begin{tabular}{|p{\tabwidth}@{}}
#1
\end{tabular}\end{change}\vskip .3cm
}
...
\begin{change}For each of the \plural{\dataVariableCount}{variable} used in the paper, JavaScript generates a \LaTeX\ header file that declares and assigns a calculated value. The header file is then included in the paper using \LaTeX's standard \texttt{\textbackslash input} command. Here is an example of one such automatic definition:
...
\initialiseBibliography{(\arabic{section}.{\alph{subsection}) \reftitle}}{\the\bibciten}{} 
